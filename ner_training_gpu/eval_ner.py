...
@torch.no_grad()
def predict(texts, model, tokenizer, max_length, batch_size=32, device="cpu"):
    out = []
...
def main():
    ...
    model = AutoModelForTokenClassification.from_pretrained(args.model).eval().to(device)
    print(f"eval device: {device}")

    def score(name, eval_texts, dump=0):
        preds = [set(p) for p in predict(eval_texts, model, tokenizer, args.max_length, device=device)]
        tp = fp = fn = 0
        errors = []
