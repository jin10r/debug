...
_UA_TABLE = str.maketrans('іїєІЇЄ', 'ииеИИЕ')
...
# футеры "подписаться"/"сообщить" уже срезаны, иначе уронили бы реальные репорты
# с подписным футером). Реальные репорты ссылок/хендлов/призывов почти не содержат.
_PROMO_RE = re.compile(
    r'(https?://|www\.|t\.me/|@[A-Za-z][\w]{3,}'
    r'|платн\w*\s+подписк|подписк\w*\s+на\s+канал|наш\s+канал|\bреклам)',
    re.IGNORECASE,
)
...
def is_promotional(text: str) -> bool:
    ...
    призывы к подписке). Цель — не ставить рекламу на карту: при срабатывании
    сообщение пропускается (skip), а не получает random-точку. Мягкую рекламу
    НЕ ловит намеренно (precision важнее) — она безвредно уйдёт в random.
    """
    if not text:
        return False
