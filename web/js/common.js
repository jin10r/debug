...
window.formatDateTime = function(dateTimeStr) {
    ...
    try {
        // Строка без явного часового пояса (naive) трактуется JS-ом как
        // ЛОКАЛЬНОЕ время устройства → конвертация в Киев даст сдвиг на
        // (TZ_устройства - Kiev), т.е. «сырое» время на чужом часовом поясе.
        // События хранятся в UTC, поэтому naive-строку ВСЕГДА интерпретируем
        // как UTC (суффикс 'Z'), и только потом конвертируем в Europe/Kiev.
        // Сначала приводим разделитель ' ' к 'T' (PSQL-стиль без T), затем
        // проверяем наличие явного offset.
